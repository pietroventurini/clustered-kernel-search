...
public class AggregatedClustersBucketBuilder extends ClusteredBucketBuilder {
	 ...
	 */
	public AggregatedClustersBucketBuilder(double bucketsRelativeSize) {
...
//		 */
//		// Mi assicuro che il numero di buckets sia almeno quanto il numero di cluster
//		int effectiveBucketsSize = (int) Math.ceil(ks.getItemsList().size() * bucketsRelativeSize);
//
...
//					for (Item it : cluster.getItems()) {
//						currentBucket.addItem(it);
//					}
...
//	}
	...
	public List<Bucket> composeBuckets(List<Set<Item>> clusters, int itemsN) {
		// Mi assicuro che il numero di buckets sia almeno quanto il numero di cluster
		int effectiveBucketsSize = (int) Math.ceil(itemsN * bucketsRelativeSize);
